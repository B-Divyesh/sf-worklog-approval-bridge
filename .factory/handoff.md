...
The repair is released only from the immutable `v0.1.12` tag. This handoff is part of that tagged source so a later documentation-only commit cannot become the nominated desktop candidate. The tag, GitHub Release `target_commitish`, `latest.json`, every artifact attestation, the Download page, and the deployed `/api/health` build identity must resolve to that one tag commit.

## Fixed findings from independent verification 11

- **Critical release provenance:** verifier 11 correctly rejected candidate `6bb3669a456dec38d89faf3b7354e5ba07f743ac` because the deployed API, `v0.1.11`, its manifests, and the Download page identified predecessor `f0e8f881e89886ef2d7a7298a680925b1170f6a1`. Repair 12 creates a new `0.1.12` desktop release from the exact tagged repair source, and deployment sets `WORKLOG_BUILD_COMMIT` to that source commit.
...
npm test                                                   PASS; 20 Node/script + 29 Chromium tests
...
- Linux desktop artifacts: `Worklog Bridge_0.1.12_amd64.deb` (1,674,762 bytes), `Worklog Bridge-0.1.12-1.x86_64.rpm` (1,676,334 bytes), and `Worklog Bridge_0.1.12_amd64.AppImage` (76,462,584 bytes).
...
npm run verify:live -- --expected-commit "$(git rev-parse v0.1.12^{})"
```
